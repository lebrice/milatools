...
from typing_extensions import Literal, Required, TypedDict, TypeGuard
...
class SshConfigEntry(TypedDict, total=False):
    ...
    Created from https://man.openbsd.org/OpenBSD-6.2/ssh_config
...
_ssh_config_entry_keys = (
    SshConfigEntry.__optional_keys__ | SshConfigEntry.__required_keys__
)
_ssh_config_entry_keys_lower = {key.lower() for key in _ssh_config_entry_keys}
_lower_to_upper_map = {k.lower(): k for k in _ssh_config_entry_keys}


def _is_valid_ssh_config(config_entry: dict) -> TypeGuard[SshConfigEntry]:
    # TODO: It's actually fine for the SshConfig entries to be in either lowercase or uppercase.
    # Seems like people use UpperCase more often though.
    return all(key in _ssh_config_entry_keys for key in config_entry.keys())


def _invalid_keys(config_entry: dict[str, Any]) -> set[str]:
    return {
        key
        for key in config_entry
        if key.lower() not in _ssh_config_entry_keys_lower
    }


def to_entry(entry: dict[str, Any]) -> SshConfigEntry:
    if _is_valid_ssh_config(entry):
        return entry

    invalid_keys = _invalid_keys(entry)
    if len(invalid_keys) == 1:
        raise ValueError(f"Invalid key in SSH config: {invalid_keys.pop()!r}")
    elif invalid_keys:
        raise ValueError(
            f"Invalid keys in SSH config entry: {sorted(invalid_keys)}"
        )
    ...
    return SshConfigEntry(
        {_lower_to_upper_map[k.lower()]: v for k, v in entry.items()}  # type: ignore
    )
